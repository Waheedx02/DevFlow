...
import { createSnippet } from "@/app/actions/snippet-action";
import { getLanguageOptions, getLanguageMeta } from "@/lib/languages";

export function AddSnippetForm({ remaining }: { remaining: number }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await createSnippet({ error: null }, formData);
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Store a secure snippet to your private developer library.
          </p>
        </div>
        <span className="text-xs font-medium text-slate-500 bg-[#13151b] border border-slate-800 rounded-full px-3 py-1.5 shrink-0">
